...
{
    if (p_dvp->baudrate) {
        return p_dvp->baudrate(p_dvp, mclk);
    }
    return RET_ERR;
...
int32 dvp_set_size(struct dvp_device *p_dvp, uint32 x_s, uint32 y_s, uint32 x_e, uint32 y_e)
{
    if (p_dvp->ioctl) {
        ...
        return p_dvp->ioctl(p_dvp, DVP_IOCTL_CMD_SET_SIZE, (uint32)size_array, 0);
    }
    return RET_ERR;
}
...
{
    if (p_dvp->ioctl) {
        return p_dvp->ioctl(p_dvp, DVP_IOCTL_CMD_SET_ADR_1, yuv_addr, 0);
    }
    return RET_ERR;
}
...
{
    if (p_dvp->ioctl) {
        return p_dvp->ioctl(p_dvp, DVP_IOCTL_CMD_SET_ADR_2, yuv_addr, 0);
    }
    return RET_ERR;
}
...
{
    if (p_dvp->ioctl) {
        return p_dvp->ioctl(p_dvp, DVP_IOCTL_CMD_RGB_2_YUV, en, 0);
    }
    return RET_ERR;
...
{
    if (p_dvp->ioctl) {
        return p_dvp->ioctl(p_dvp, DVP_IOCTL_CMD_EX_D5_D6, en, 0);
    }
    return RET_ERR;
...
{
    if (p_dvp->ioctl) {
        return p_dvp->ioctl(p_dvp, DVP_IOCTL_CMD_SET_FORMAT, format, 0);
    }
    return RET_ERR;
}
...
{
    if (p_dvp->ioctl) {
        return p_dvp->ioctl(p_dvp, DVP_IOCTL_CMD_SET_SCEN, en, 0);
    }
    return RET_ERR;
}
...
{
    if (p_dvp->ioctl) {
        return p_dvp->ioctl(p_dvp, DVP_IOCTL_CMD_SET_VSYNC_VAILD, high_valid, 0);
    }
    return RET_ERR;
}
...
{
    if (p_dvp->ioctl) {
        return p_dvp->ioctl(p_dvp, DVP_IOCTL_CMD_SET_HSYNC_VAILD, high_valid, 0);
    }
    return RET_ERR;
}
...
{
    if (p_dvp->ioctl) {
        return p_dvp->ioctl(p_dvp, DVP_IOCTL_CMD_SET_ONE_SAMPLE, en, 0);
    }
    return RET_ERR;
}
...
{
    if (p_dvp->ioctl) {
        return p_dvp->ioctl(p_dvp, DVP_IOCTL_CMD_SET_DEBOUNCE, en, pixel);
    }
    return RET_ERR;
}
...
{
    if (p_dvp->ioctl) {
        return p_dvp->ioctl(p_dvp, DVP_IOCTL_CMD_SET_YCBCR_MODE, mode, 0);
    }
    return RET_ERR;
}
...
{
    if (p_dvp->ioctl) {
        return p_dvp->ioctl(p_dvp, DVP_IOCTL_CMD_DIS_UV_MODE, en, 0);
    }
    return RET_ERR;
}
...
{
    if (p_dvp->ioctl) {
        return p_dvp->ioctl(p_dvp, DVP_IOCTL_CMD_SET_FRAME_RATE, precent, 0);
    }
    return RET_ERR;
}
...
int32 dvp_low_high_threshold(struct dvp_device *p_dvp, bool low_high)
{
    ...
    if (p_dvp->ioctl) {
        return p_dvp->ioctl(p_dvp, DVP_IOCTL_CMD_SET_THRESHOLD, dlt, dht);
    }
    return RET_ERR;
}
...
{
    if (p_dvp->ioctl) {
        return p_dvp->ioctl(p_dvp, DVP_IOCTL_CMD_SET_JPEG_LEN, len, 0);
    }
    return RET_ERR;
...
{
    if (p_dvp->request_irq) {
        return p_dvp->request_irq(p_dvp, irq_flags, irq_hdl, irq_data);
    }
    return RET_ERR;
}
...
{
    if (p_dvp->release_irq) {
        return p_dvp->release_irq(p_dvp, irq_flags);
    }
    return RET_ERR;
}
