...
static uint32_t file_mode (const char *mode)
{
  ...
  if (mode[0] == 'w') return FA_READ | FA_WRITE | FA_CREATE_NEW;
  if (mode[0] == 'a') return FA_READ | FA_WRITE | FA_OPEN_ALWAYS ;
...
F_FILE *osal_fopen(const char *filename,const char *mode)
{
	...
	uint32_t res = f_open (fp, filename, mod);
	if(res == FR_OK)
	{
		return fp;
	}
...
uint32_t osal_fread2 (char *buf, uint32_t is, uint32_t s, void *fp)
{
  ...
  FRESULT res;
	...
	{
		temp_buf = malloc(is*s+4);	//申请一个缓冲buf,需要足够大小
		if(!temp_buf)
		{
...
//			printf("1fp addr =  %d\r\n",fp);
		...
		temp_buf = malloc(is*s);//写入不超过is*s
		//空间不足
		if(!temp_buf)
		{
...
{
  struct diriter *dir;
  dir = malloc( sizeof(struct diriter) );
  if (!dir) return NULL;
