...
static int32 hgcapture_v0_config(struct hgcapture_v0 *dev, enum capture_channel channel, struct hgcapture_v0_config *config) {
    ...
    if ((dev->channel[channel_to_reg]) && (((struct timer_device *)(dev->channel[channel_to_reg]))->ioctl)) {
        ret_val = ((struct timer_device *)(dev->channel[channel_to_reg]))->ioctl(((struct timer_device *)dev->channel[channel_to_reg]), HGCAPTURE_V0_FUNC_CMD_MASK, (uint32)p_config, 0);
    } else {
        ret_val = RET_ERR;
    }
...
int32 hgcapture_v0_attach(uint32 dev_id, struct hgcapture_v0 *capture) {
    ...
    capture->dev.init        = hgcapture_v0_init;
    capture->dev.deinit      = hgcapture_v0_deinit;
    capture->dev.start       = hgcapture_v0_start;
    capture->dev.stop        = hgcapture_v0_stop;
    capture->dev.ioctl       = hgcapture_v0_ioctl;
    capture->dev.request_irq = hgcapture_v0_request_irq;
    capture->dev.release_irq = hgcapture_v0_release_irq;
