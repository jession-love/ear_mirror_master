...
static inline void hgi2s_v0_set_mclk(struct hgi2s_v0_hw *p_i2s, uint32 freq) {
    ...
    if (HG_IIS0_BASE == (uint32)p_i2s) {
        ...
        mclk_div = (peripheral_clock_get(HG_APB0_PT_IIS0) / (freq*256));
        #endif
...
int32 hgi2s_v0_open(struct i2s_device *i2s, enum i2s_mode mode, enum i2s_sample_freq frequency, enum i2s_sample_bits bits)
{
    ...
    hw->BAUD    = (64 / i2s_sample_bits_to_reg) - 1;
...
int32 hgi2s_v0_attach(uint32 dev_id, struct hgi2s_v0 *i2s) {
    ...
    i2s->irq_data        = 0;
    i2s->duplex_en       = 0;
    i2s->dev.open        = hgi2s_v0_open;
    i2s->dev.close       = hgi2s_v0_close;
    i2s->dev.write       = hgi2s_v0_write;
    i2s->dev.read        = hgi2s_v0_read;
    i2s->dev.request_irq = hgi2s_v0_request_irq;
    i2s->dev.release_irq = hgi2s_v0_release_irq;
    i2s->dev.ioctl       = hgi2s_v0_ioctl;
