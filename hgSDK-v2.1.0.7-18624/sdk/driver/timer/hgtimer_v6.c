...
static inline int32 hgtimer_v6_pwm_func_init(struct hgtimer_v6 *dev, struct hgpwm_v0_config *p_config) {
    ...
    /* config period */
    hw_suptmrx->PR  = ( (p_config->period) * (peripheral_clock_get(HG_APB1_PT_STMR) / 1000000) );

    /* config compare */
    hw_suptmrx->CMP = ( (p_config->duty) * (peripheral_clock_get(HG_APB1_PT_STMR) / 1000000) );
...
static int32 hgtimer_v6_counter_func_start(struct timer_device *timer, uint32 tmo_us, timer_cb_hdl cb, uint32 cb_data) {
    ...
    switch (dev->type) {
        ...
        case (HGTIMER_V6_TYPE_ONCE):
            ...
            /* config period */
            hw_suptmrx->PR = ((tmo_us) * (peripheral_clock_get(HG_APB1_PT_STMR) / 1000000));
...
int32 hgtimer_v6_attach(uint32 dev_id, struct hgtimer_v6 *timer) {
    ...
    timer->irq_data         = 0;
    timer->dev.open         = hgtimer_v6_counter_func_open;
    timer->dev.close        = hgtimer_v6_counter_func_close;
    timer->dev.start        = hgtimer_v6_counter_func_start;
    timer->dev.stop         = hgtimer_v6_counter_func_stop;
    timer->dev.ioctl        = hgtimer_v6_counter_func_ioctl;
    timer->dev.request_irq  = NULL;
