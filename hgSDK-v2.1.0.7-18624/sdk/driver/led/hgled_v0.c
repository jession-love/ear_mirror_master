...
static int32 hgled_v0_init(struct led_device *led, enum led_work_mode work_mode, enum led_seg_cnt seg_cnt, enum led_com_cnt com_cnt) {
...
static int32 hgled_v0_open(struct led_device *led, uint32 num, enum led_com_cnt pos) {
    ...
    led_ptr = (uint32)(&(hw->DATA0)) + ((i / 2)*0x4);
    data_temp = *((uint32 *)led_ptr);
    if (i % 2) {
        data_temp = (data_temp &~ (0xFF << 16)) | (num_tbl[e] << 16);
    } else {
        data_temp = (data_temp &~ (0xFF      )) | (num_tbl[e]      );
    }

    *((uint32 *)led_ptr) = data_temp;
...
int32 hgled_v0_attach(uint32 dev_id, struct hgled_v0 *led) {
    ...
    led->irq_hdl         = NULL;
    led->irq_data        = 0;
    led->dev.init        = hgled_v0_init;
    led->dev.open        = hgled_v0_open;
    led->dev.close       = hgled_v0_close;
    led->dev.ioctl       = hgled_v0_ioctl;
    led->dev.request_irq = hgled_v0_request_irq;
    led->dev.release_irq = hgled_v0_release_irq;
