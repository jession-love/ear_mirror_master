...
{
	audio_8311_config *priv = (audio_8311_config*)irq_data;
	void *buf;
	int buf_size;
...
int audio_8311_driver_install(audio_8311_config *play_config,audio_8311_config *mic_config)
{
	int res = RET_ERR;
	int open_res = 0;
	int duplex_en = 0;
	struct i2s_device *i2s_dev;
	audio_8311_config *config;
	int i2s_devid;
	int		sample_freq = ~0;
	int		sample_bit  = ~0;
	int 	data_format = ~0;
	...
	{
		goto audio_8311_driver_install_end;
	}
	//需要同步
	if(play_config  && mic_config)
	{
		...
		memcpy(iis_sram_start,iis_start,iis_end-iis_start);
	...
	res = RET_OK;
	audio_8311_driver_install_end:
	return res;
}

int audio_8311_driver_uninstall(audio_8311_config *play_config,audio_8311_config *mic_config)
{
	int res = RET_ERR;
	int i2s_devid;
	struct i2s_device *i2s_dev;
	...
	if(play_config && play_config->i2s_devid != ~0)
	{
		...
		if(!i2s_dev)
		{
			goto audio_8311_driver_uninstall_end;
		}
		i2s_close(i2s_dev);
	}
	...
	res = I2S_OK;
	audio_8311_driver_uninstall_end:
	return res;
