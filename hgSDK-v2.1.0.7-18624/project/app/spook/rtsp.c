...
static int handle_DESCRIBE( struct req *req )
{
	...
	{
		printf("sdp:%x  %x\n",(int)sess->get_sdp,(int)sess->get_sdp( sess, sdp, &sdp_len, req->req->sl.req.uri ));
		rtsp_send_error( req, 404, "Not Found" );
	}
