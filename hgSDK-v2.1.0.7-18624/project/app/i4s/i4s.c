...
int i4s_camera_init(void)
{
   ...
   I4SC_WARN("chengjie+++++++++++++++++++++++++%s %s\r\n", __DATE__, __TIME__);
	...
	/* op_cb callback_cj=dp_sendto;
	i4s_op_callback_register(callback_cj); */
...
int i4s_handle_event(int event)
{
	...
	//I4SC_PRT("99999999999999");
	if(op_callback&&(event==7)){
		//I4SC_PRT("9999999lai le ---====");
...
int dp_sendto(int event)
{
	...
	if(op_sock!=0)
	{
	  ...
	  sendto(op_sock,(void *)prohdr,snd_len,0,(struct sockaddr *)&cli_addr,addr_len);
	  i--;
