...
static int handle_GET( struct req *req )
{
	...
	if( loc->streaming )
	{
		...
		spook_log( SL_DEBUG, "send_multipart_frame" );
	...
	} else
	{
		...
		req->conn->drop_after = 1;
		spook_log( SL_DEBUG, "send_frame_with_jfif" );
...
{
	//spook_log (SL_DEBUG, "http jpeg_next_frame");
	struct http_location *loc = (struct http_location *)d;
	struct http_stream_session *s;

	//spook_log( SL_DEBUG, "jpeg_next_frame" );
...
static int set_input( char *name, void *d )
{
	struct http_location *loc = (struct http_location *)d;
	int format = FORMAT_JPEG;

  	spook_log (SL_DEBUG, "http set_input %s, format is %d", name, format);

	if( ! ( loc->input = connect_to_stream( name,
					jpeg_next_frame, loc ) ) )
	{
		spook_log( SL_ERR,
			"http output: unable to connect to stream \"%s\"", name );
		return -1;
	}
	return 0;
}

static int set_path( char *name, void *d )
{
	struct http_location *loc = (struct http_location *)d;

	strcpy( loc->node.path, name );
	return 0;
}

static int set_mode( char *name, void *d )
{
	struct http_location *loc = (struct http_location *)d;

  	spook_log (SL_DEBUG, "http set_mode %s", name);

	if( ! strcasecmp( name, "single" ) )
		loc->streaming = 0;
	else if( ! strcasecmp( name, "stream" ) )
		loc->streaming = 1;
	else
	{
		spook_log( SL_ERR,
...
int http_init(void)
{
	...
	loc =start_block();
	set_path( HTTP_PATTH, loc);
	set_input( HTTP_INPUT_NAME, loc );
	set_mode( HTTP_MODE, loc);
	end_block(loc);
