...
static int32 hgpwm_v0_config(struct hgpwm_v0 *dev, enum pwm_channel channel, struct hgpwm_v0_config *config) {
    ...
    if ((dev->channel[channel_to_reg]) && (((struct timer_device *)(dev->channel[channel_to_reg]))->ioctl)) {
        ret_val = ((struct timer_device *)(dev->channel[channel_to_reg]))->ioctl(((struct timer_device *)dev->channel[channel_to_reg]), HGPWM_V0_FUNC_CMD_MASK, (uint32)p_config, 0);
    } else {
        ret_val = RET_ERR;
    }
...
static int32 hgpwm_v0_init(struct pwm_device *pwm, enum pwm_channel channel, uint32 period_us, uint32 h_duty_us) {
    ...
    /* config pwm */
    config.period   = period_us;
    config.duty     = h_duty_us;
...
int32 hgpwm_v0_attach(uint32 dev_id, struct hgpwm_v0 *pwm) {
    ...
    pwm->dev.init        = hgpwm_v0_init;
    pwm->dev.deinit      = hgpwm_v0_deinit;
    pwm->dev.start       = hgpwm_v0_start;
    pwm->dev.stop        = hgpwm_v0_stop;
    pwm->dev.ioctl       = hgpwm_v0_ioctl;
    pwm->dev.request_irq = hgpwm_v0_request_irq;
    pwm->dev.release_irq = hgpwm_v0_release_irq;
